...
final class CommonMenuTest extends TestCase {
  ...
  public function testMenuTrees(): void {
    ...
    static::assertEquals($item1, $menuData[$menuTreesResult->menuTrees[0]->link]);
    static::assertEquals($item2, $menuData[$menuTreesResult->menuTrees[1]->link]);
    static::assertEquals($item2a, $menuData[$menuTreesResult->menuTrees[1][0]->link]);
    static::assertEquals($item2b, $menuData[$menuTreesResult->menuTrees[1][1]->link]);
    static::assertEquals($item2ba, $menuData[$menuTreesResult->menuTrees[1][1][0]->link]);
    static::assertEquals($item2bb, $menuData[$menuTreesResult->menuTrees[1][1][1]->link]);
    static::assertEquals($item2c, $menuData[$menuTreesResult->menuTrees[1][2]->link]);
    static::assertEquals($item3, $menuData[$menuTreesResult->menuTrees[2]->link]);
    static::assertEquals($item3a, $menuData[$menuTreesResult->menuTrees[2][0]->link]);
  }

  public function testMenusUnderActiveTrailRootItem(): void {
    $cacheContextsManager = \Mockery::mock(CacheContextsManager::class);
    $cacheContextsManager->expects('assertValidTokens')->andReturn(TRUE);

    $menuLinkPluginForActiveTrailRoot = \Mockery::mock(\Drupal\Core\Menu\MenuLinkInterface::class);
    $menuLinkPluginForActiveTrailRoot->expects('getPluginId')
      ->andReturn('fakepluginuuid');
    ...
    $menuTree->expects('build')->andReturnValues([
      [
        '#cache' => [
          'contexts' => [
            'user.permissions',
        ...
        '#items' => [
          ...
          'menu_link_content:2' => [
            'original_link' => $menuLinkPluginForActiveTrailRoot,
            'in_active_trail' => TRUE,
            'title' => 'Item 2',
            'url' => $url,
            'below' => [],
      ...
      [
        ...
        '#items' => [
          ...
          'menu_link_content:2' => [
            ...
            'url' => $url,
            'below' => [
              'menu_link_content:2A' => [
                'in_active_trail' => FALSE,
                'title' => 'Item 2A',
                'url' => $url,
                'below' => [],
              ],
              'menu_link_content:2B' => [
                'in_active_trail' => TRUE,
                'title' => 'Item 2B',
                'url' => $url,
                'below' => [
                  'menu_link_content:2BA' => [
                    'in_active_trail' => FALSE,
                    'title' => 'Item 2BA',
                    'url' => $url,
                    'below' => [],
                  ],
                  'menu_link_content:2BB' => [
                    'in_active_trail' => TRUE,
                    'title' => 'Item 2BB',
                    'url' => $url,
                    'below' => [],
    ...
    $menuTreesActiveTrailRootResult = $service->menusUnderActiveTrailRootItem($specification);

    // Root:
    static::assertEquals(Common\Atom\Link\Link::create(
      'Item 2',
      $url,
    ), $menuTreesActiveTrailRootResult->rootLink);

    static::assertEquals(Common\Atom\Link\Link::create(
      'Item 2BB',
      $url,
    ), $menuTreesActiveTrailRootResult->result->activeLink);
    ...
    static::assertEquals(
      (new CacheableMetadata())
        ...
        ->setCacheTags(['config:system.menu.main'])
        ->setCacheMaxAge(-1),
      $menuTreesActiveTrailRootResult->result->cachability,
    );
    ...
    static::assertEquals($expectedMenuTree, $menuTreesActiveTrailRootResult->result->menuTrees);
  }
